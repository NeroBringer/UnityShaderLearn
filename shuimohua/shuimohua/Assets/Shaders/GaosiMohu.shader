	...
	SubShader 
	{
		pass	
		{
			...
			struct v2f_blur
			{
				...
				float4 uv01 : TEXCOORD1;	 //一个victor4存储两个纹理坐标	
				float4 uv23 : TEXCOORD2;	 //一个victor4存储两个纹理坐标
				float4 uv45 : TEXCOORD3;	 //一个victor4存储两个纹理坐标
			};
			
			sampler2D _MainTex;
			//XX_TexelSize，XX纹理的像素相关大小width，height对应纹理的分辨率，x = 1/width, y = 1/height, z = width, w = height
			float4 _Mainx_TexelSize;
			...
			v2f_blur vert_blur(appdata_img v)
			{
		        ...
		        _offsets *= _Mainx_TexelSize.xyxy;
